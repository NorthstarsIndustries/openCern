...
export function isSimBuilt() {
    return existsSync(SIM_BINARY) || existsSync(SIM_LOCAL_BINARY);
}
function getSimBinary() {
    if (existsSync(SIM_BINARY))
        return SIM_BINARY;
    if (existsSync(SIM_LOCAL_BINARY))
        return SIM_LOCAL_BINARY;
    return SIM_BINARY;
}
export function isSimSourcePresent() {
    return existsSync(join(SIM_SOURCE, 'CMakeLists.txt'));
}
export function checkBuildDeps() {
...
export function buildSim(onOutput) {
...
export function launchSim(filePath, event) {
    if (!isSimBuilt()) {
    ...
    try {
        ...
        return { success: false, message: `Could not launch viewer: ${err.message}` };
    }
}
