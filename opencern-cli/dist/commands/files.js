...
// ── Cat ──────────────────────────────────────────────────
export function catFile(filePath, maxLines = 50) {
...
// ── Grep ─────────────────────────────────────────────────
export function grepFile(pattern, filePath) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const content = readFileSync(filePath, 'utf-8');
        ...
        if (matches.length === 0)
            return [`  No matches for "${pattern}" in ${filePath}`];
        return [
            '',
...
export function findFiles(pattern) {
    ...
    if (!existsSync(dataDir))
        return [`  [-] Data directory not found: ${dataDir}`];
    const results = [];
...
// ── Diff ─────────────────────────────────────────────────
export function diffFiles(file1, file2) {
    if (!existsSync(file1))
...
// ── Clean ────────────────────────────────────────────────
export function cleanFiles(dryRun = true) {
    const dataDir = config.get('dataDir');
...
// ── Cache ────────────────────────────────────────────────
export function cacheInfo(clear = false) {
    const cacheDir = join(homedir(), '.opencern', 'cache');
    ...
    if (clear && fileCount > 0) {
        try {
            ...
            return [`  [-] Could not clear cache: ${err.message}`];
        }
    }
    return [
        '',
