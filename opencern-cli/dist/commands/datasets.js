...
export function formatDatasetList(datasets) {
    ...
    for (const ds of datasets) {
        ...
        lines.push(`  ${ds.name.padEnd(30)} ${ds.type.padEnd(6)} ${size.padEnd(10)} ${events.padEnd(10)} ${modified}`);
    }
    lines.push('');
...
// ── Stats ────────────────────────────────────────────────
export function getDatasetStats(filePath) {
    ...
    try {
        ...
        if (Object.keys(numericFields).length > 0) {
            ...
            for (const [field, values] of Object.entries(numericFields)) {
                ...
                lines.push(`  ${field.padEnd(16)} ${min.toFixed(3).padEnd(12)} ${max.toFixed(3).padEnd(12)} ${mean.toFixed(3).padEnd(12)} ${std.toFixed(3)}`);
            }
        }
        ...
        return [`  [-] Could not parse: ${err.message}`];
    }
}
// ── Histogram ────────────────────────────────────────────
export function renderHistogram(filePath, field) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
        ...
        if (values.length === 0)
            return [`  [-] No numeric values for field "${field}"`];
        const min = Math.min(...values);
        const max = Math.max(...values);
        const bins = 20;
...
// ── Scatter Plot ─────────────────────────────────────────
export function renderScatterPlot(filePath, xField, yField) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
        ...
        if (points.length === 0)
            return [`  [-] No numeric values for fields "${xField}", "${yField}"`];
        const width = 60;
        const height = 20;
...
// ── Head/Tail ────────────────────────────────────────────
export function headEvents(filePath, n = 10) {
    return showEvents(filePath, 0, n, 'head');
}
export function tailEvents(filePath, n = 10) {
    return showEvents(filePath, -n, n, 'tail');
}
function showEvents(filePath, offset, count, label) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
...
// ── Describe ─────────────────────────────────────────────
export function describeDataset(filePath) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    ...
    if (filePath.endsWith('.json')) {
        try {
            ...
            if (raw.experiment)
                lines.push(`  Experiment: ${raw.experiment}`);
            if (raw.metadata?.energy)
                lines.push(`  Energy:    ${raw.metadata.energy}`);
            if (raw.metadata?.year)
                lines.push(`  Year:      ${raw.metadata.year}`);
            if (Array.isArray(events) && events.length > 0) {
...
// ── Filter ───────────────────────────────────────────────
export function filterEvents(filePath, filters) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
...
// ── Sample ───────────────────────────────────────────────
export function sampleEvents(filePath, n = 1000) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
...
// ── Export ────────────────────────────────────────────────
export function exportDataset(filePath, format) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
...
// ── Merge ────────────────────────────────────────────────
export function mergeDatasets(file1, file2) {
    if (!existsSync(file1))
...
// ── Correlate ────────────────────────────────────────────
export function correlateFields(filePath) {
    if (!existsSync(filePath))
        return [`  [-] File not found: ${filePath}`];
    try {
        const raw = JSON.parse(readFileSync(filePath, 'utf-8'));
        ...
        const numFields = [];
        if (events.length > 0) {
            for (const [key, val] of Object.entries(events[0])) {
                if (typeof val === 'number')
                    numFields.push(key);
            }
        }
        if (numFields.length < 2)
            return ['  [-] Need at least 2 numeric fields for correlation'];
        const data = {};
