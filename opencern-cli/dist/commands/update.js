...
function readCache() {
    try {
        ...
        if (Date.now() - data.lastCheck > CACHE_TTL)
            return null;
        return data;
    }
    catch {
...
async function checkDockerDigests() {
...
export async function checkForUpdates(useCache = true) {
    // Check cache first
    if (useCache) {
...
export function getUpdateBanner() {
    ...
    const parts = [];
    if (cached.cliUpdate)
        parts.push(`CLI v${cached.latestVersion}`);
    if (cached.dockerUpdate)
