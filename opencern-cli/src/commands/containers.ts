...
export function getLogs(service?: string, tail = 50): string[] {
  ...
  try {
    ...
    if (logLines.length === 0) return [`  No logs for ${containerName}`];
    ...
    return [`  [-] Could not get logs for ${containerName}: ${(err as Error).message}`];
  }
}
...
export async function restartService(service?: string): Promise<string[]> {
  ...
  try {
    ...
    return [`  [-] Could not restart ${containerName}: ${(err as Error).message}`];
  }
}
...
export async function stopAll(): Promise<string[]> {
  ...
  try {
    ...
    return [`  [-] Could not stop containers: ${(err as Error).message}`];
  }
}

// ── Pull ─────────────────────────────────────────────────

export async function pullImages(onProgress: (image: string) => void): Promise<string[]> {
  if (!docker.isDockerRunning()) {
    return ['  [-] Docker is not running'];
  }
...
export function containerTop(): string[] {
  if (!docker.isDockerRunning()) {
    return ['  [-] Docker is not running'];
  }
  ...
  try {
    const output = execSync(
      'docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}\t{{.PIDs}}" 2>/dev/null | grep opencern',
      { encoding: 'utf-8', timeout: 10000 },
    );

    if (!output.trim()) return ['  No running OpenCERN containers'];
...
export function networkInfo(): string[] {
  if (!docker.isDockerRunning()) {
    return ['  [-] Docker is not running'];
  }
