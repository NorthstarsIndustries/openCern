...
export function envInfo(): string[] {
  ...
  if (dockerVersion) lines.push(`  Docker         ${dockerVersion.replace('Docker version ', '').trim()}`);

  const pythonVersion = safeExec('python3 --version') || safeExec('python --version');
  if (pythonVersion) lines.push(`  Python         ${pythonVersion.replace('Python ', '').trim()}`);
...
export async function versionInfo(): Promise<string[]> {
  ...
  if (dockerVer) lines.push(`  Docker       ${dockerVer.replace('Docker version ', '').split(',')[0]}`);
...
export function aboutInfo(): string[] {
